...
export async function submitApplication(req, res) {
    ...
    try {
        ...
        // Award points for submitting application (community participation)
        await PointsController.addPointsInternal(
            user_id,
            25,
    ...
    try {
        ...
        try {
            if (status === "Accepted") {
                ...
                // Award points for task completion when application is accepted
                await PointsController.addPointsInternal(
                    applicant.id,
                    100,
                    'task_completed',
                    `Job application accepted: ${job.title}`,
                    parseInt(id),
                    'job_application'
                );
                ...
                await NotificationModel.create({
                    user_id: applicant.id,
                    title: "Application Escrowed",
                    message: `Your application for "${job.title}" has been escrowed by ${jobPoster.name}`,
                    type: "application_status",
                    reference_id: parseInt(id),
                    reference_type: "job_application",
                });

                console.log(`Notification created for completed application: ${id}`);
