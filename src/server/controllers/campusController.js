...
export const createPost = async (req, res) => {
    try {
        ...
        // Award points for community participation (posting content)
        const pointsForPost = type === 'poll' ? 15 : 10;
        await PointsController.addPointsInternal(
            req.user.id,
            pointsForPost,
            'community_participation',
            `Created ${type} post: ${title}`,
            postId,
            'campus_post'
        );
...
export const createComment = async (req, res) => {
    try {
        ...
        // Award points for community participation (commenting)
        await PointsController.addPointsInternal(
            req.user.id,
            5,
            'community_participation',
            'Posted comment on campus',
            commentId,
            'campus_comment'
        );
...
export const togglePostLike = async (req, res) => {
    try {
        ...
        // Award points for community participation (engaging with posts)
        if (isLiked) {
            await PointsController.addPointsInternal(
                req.user.id,
                2,
