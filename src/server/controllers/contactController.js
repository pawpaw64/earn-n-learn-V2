...
export const updateSkillContactStatus = async (req, res) => {
    ...
    try {
        ...
        // Award points for successful skill contact agreements
        if (status === 'Agreement Reached') {
            await PointsController.addPointsInternal(
                contact.provider_id,
                25,
                'community_participation',
                `Reached agreement for skill: ${contact.skill_name}`,
                parseInt(id),
                'skill_contact'
            );
        }
...
export const updateMaterialContactStatus = async (req, res) => {
    ...
    try {
        ...
        // Award points for successful material contact agreements
        if (status === 'Agreement Reached') {
            await PointsController.addPointsInternal(
                contact.seller_id,
                25,
                'lending_activity',
                `Reached agreement for material: ${contact.title}`,
                parseInt(id),
                'material_contact'
            );
        }
